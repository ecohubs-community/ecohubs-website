<script lang="ts">
	import { Network, Wifi, Battery, Check, X, LayoutGrid, Leaf, Coins } from 'lucide-svelte';
	...
	const tools: EcosystemTool[] = [
		...
		},
		{
			...
			title: 'Discord',
			description:
				'Real-time community chat. Public channels for open discussion, private member channels for deeper collaboration. [Third-party tool]',
		...
		},
		{
			...
			title: 'Offcoin',
			description:
				'Off-chain tokens reward collaboration. XP unlocks permissions and responsibilities. Built into Puckstack and EcoHubsOS.',
...
<section
	id="ecosystem"
	class="py-16 md:py-24 relative bg-gradient-to-b from-ecohubs-base via-emerald-50/30 to-ecohubs-base overflow-hidden"
>
	<div class="absolute inset-0 pointer-events-none hidden md:block">
		<div
			class="absolute top-0 left-1/2 -translate-x-1/2 w-[1000px] h-[600px] bg-emerald-100/40 rounded-full blur-3xl opacity-50"
		></div>
	</div>
	...
	<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
		<div class="text-center mb-8 md:mb-12 max-w-3xl mx-auto">
			...
			<h3
				class="font-serif text-3xl md:text-4xl lg:text-5xl text-ecohubs-deep mb-4 md:mb-6 leading-tight"
			>
				The <em class="font-story italic font-normal text-ecohubs-primary">operating system</em><br
		...
		<div class="md:hidden grid grid-cols-2 gap-3 mb-8">
            ...
            {activeTool?.title === tool.title
				...
				>
					<div
						class="w-10 h-10 rounded-xl {tool.iconBg} {tool.iconColor} flex items-center justify-center mb-3"
					>
						<Icon icon={tool.icon} class="w-5 h-5" />
					</div>
					<h4 class="font-serif text-sm text-ecohubs-deep mb-1">{tool.title}</h4>
					<span class="text-[10px] uppercase tracking-[0.18em] text-stone-500 font-medium"
						>{tool.category}</span
					>
				</button>
			{/each}
		...
		{#if activeTool}
			<div
				class="md:hidden mb-8 p-6 rounded-2xl bg-white border border-stone-200/70 shadow-lg"
				transition:fly={{ y: 10, duration: 200 }}
			>
				<div class="flex items-start gap-4 mb-4">
					<div
						class="w-14 h-14 rounded-2xl {activeTool.iconBg} {activeTool.iconColor} flex items-center justify-center shrink-0"
					>
				...
				<div class="flex items-center gap-4 mt-4">
					...
					{#if activeTool.link}
						<a
							href={activeTool.link.href}
							target={activeTool.link.target}
							class="text-xs font-semibold text-emerald-700 hover:text-emerald-900 underline decoration-emerald-300 underline-offset-4"
						>
		...
		<div class="relative w-full max-w-5xl mx-auto hidden md:block">
			<div
				class="relative bg-ecohubs-deep rounded-[2rem] p-2 shadow-2xl border border-emerald-900/40 ring-1 ring-emerald-900/20 transform transition-transform duration-700"
			>
				<div
					class="relative w-full aspect-[16/10] md:aspect-[16/9] bg-gradient-to-br from-ecohubs-ivory to-emerald-50 rounded-[1.5rem] overflow-hidden flex flex-col shadow-inner"
				>
					<div
						class="w-full h-9 bg-white/60 backdrop-blur-md flex items-center justify-between px-4 z-20 border-b border-stone-200/60"
					>
						<div
							class="flex items-center gap-2 text-[10px] md:text-xs font-medium text-ecohubs-deep"
						>
						...
						<div class="flex items-center gap-3 text-ecohubs-deep">
							<span
								class="inline-flex items-center gap-1 text-[10px] font-medium px-2 py-0.5 rounded-full bg-emerald-50 border border-emerald-200/70 text-emerald-800"
							>
								<Leaf class="w-3 h-3" strokeWidth={2} />
								<span>1322 XP</span>
							</span>
							<span
								class="inline-flex items-center gap-1 text-[10px] font-medium px-2 py-0.5 rounded-full bg-amber-50 border border-amber-200/70 text-amber-800"
							>
					...
					<div class="flex-1 relative p-4 md:p-8">
						<div
							class="grid grid-cols-1 sm:grid-cols-2 md:flex md:flex-col gap-4 absolute top-8 left-4 md:left-8 z-10 w-full max-w-[280px] pointer-events-none"
						>
							<div
								class="bg-white/85 backdrop-blur-lg p-4 rounded-2xl shadow-lg border border-stone-200/60 pointer-events-auto transform hover:scale-[1.02] transition-transform"
							>
								<div class="flex items-center gap-3 mb-3">
									<div
										class="w-10 h-10 rounded-full overflow-hidden ring-2 ring-white shadow-md bg-emerald-100"
									>
										<img
											src="https://i.pravatar.cc/150?u=ecohubs"
											alt="User"
									...
									<div>
										...
										<div class="text-[10px] text-stone-500 font-medium font-story italic">
											Level 3 · Steward
										</div>
									</div>
								</div>
								<div class="space-y-2">
									<div
										class="flex justify-between items-center text-[10px] text-stone-500 font-medium tracking-[0.18em] uppercase"
									>
										<span>Reputation</span>
										<span class="text-emerald-700 tracking-normal normal-case font-semibold"
											>1322 XP</span
										>
									</div>
									<div class="h-1.5 w-full bg-stone-200/80 rounded-full overflow-hidden">
										<div
											class="h-full w-[72%] bg-gradient-to-r from-emerald-500 to-emerald-600"
										></div>
									</div>
								</div>
							</div>
							...
							>
								<div class="flex items-center justify-between mb-3">
									...
									<div
										class="w-5 h-5 rounded-full bg-emerald-100 flex items-center justify-center text-emerald-700"
									>
								...
								<div class="space-y-3 relative">
									...
									<div class="flex items-center gap-3">
										<div
											class="w-5 h-5 rounded-full bg-emerald-600 flex items-center justify-center border-2 border-white shadow-sm z-10"
										>
									...
									<div class="flex items-center gap-3">
										<div
											class="w-5 h-5 rounded-full bg-white border-2 border-emerald-600 flex items-center justify-center shadow-sm z-10"
										>
											<div class="w-2 h-2 rounded-full bg-emerald-600 animate-pulse"></div>
										</div>
										<span class="text-[11px] font-medium text-ecohubs-deep"
											>Claim Discord Member status</span
										>
									</div>

									<div class="flex items-center gap-3">
										<div
											class="w-5 h-5 rounded-full bg-stone-100 border-2 border-white flex items-center justify-center z-10"
										>
						...
						{#if activeTool}
							<div
								class="absolute inset-0 z-40 flex items-center justify-center p-4 md:p-12"
							...
							>
								...
								>
									<div
										class="h-10 bg-ecohubs-ivory/70 border-b border-stone-200/70 flex items-center justify-between px-4 shrink-0"
									>
										...
										<span
											class="text-xs font-medium text-stone-500 flex items-center gap-1.5 font-serif"
										>
									...
									<div class="p-8 flex flex-col items-center text-center relative overflow-hidden">
										<div
											class="absolute top-0 left-0 w-full h-32 bg-gradient-to-b {activeTool.iconBg.replace(
												'bg-',
												'from-'
											)}/50 to-transparent opacity-30"
										></div>

										<div
											class="relative w-20 h-20 {activeTool.iconBg} rounded-2xl flex items-center justify-center shadow-lg mb-6 ring-4 ring-white"
										>
										...
										<span
											class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-medium tracking-[0.18em] uppercase bg-emerald-50 text-emerald-800 mb-6 border border-emerald-100"
										>
											{activeTool.category}
										</span>
						...
						{#if !activeTool}
							...
							>
								<div class="text-center opacity-70">
									...
									<p
										class="font-story italic text-stone-600 text-sm bg-white/60 px-4 py-2 rounded-full backdrop-blur-sm shadow-sm inline-block border border-stone-200/50"
									>
					...
					<div
						class="h-20 md:h-24 w-full flex items-end justify-center pb-4 md:pb-6 z-50 pointer-events-none"
					>
						<div
						...
						>
							{#each tools.filter((t) => !t.highlight) as tool}
								<div class="relative group flex flex-col items-center">
									<div
										class="absolute -top-12 left-1/2 -translate-x-1/2 bg-ecohubs-deep text-ecohubs-ivory text-[10px] font-medium tracking-wide py-1 px-2.5 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none mb-2 font-serif"
									>
										{tool.title}
										<div
											class="absolute -bottom-1 left-1/2 -translate-x-1/2 w-2 h-2 bg-ecohubs-deep rotate-45"
										></div>
									</div>
                                ...
                                {activeTool?.title === tool.title
											? 'ring-2 ring-offset-2 ring-offset-transparent ring-emerald-500 -translate-y-2'
											: ''}"
									>
										<div
											class="absolute inset-0 rounded-xl md:rounded-2xl {tool.iconBg} opacity-80 group-hover:opacity-100 transition-opacity"
										></div>
										<Icon
											icon={tool.icon}
											class="w-5 h-5 md:w-6 md:h-6 {tool.iconColor} relative z-10"
										/>
							...
							<button
								class="w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl bg-white/60 hover:bg-white flex items-center justify-center transition-all hover:-translate-y-1 cursor-pointer"
							>
				...
				<div
					class="absolute bottom-[-14px] left-1/2 -translate-x-1/2 w-[120px] h-[14px] bg-ecohubs-deep rounded-b-xl opacity-50 blur-sm"
				></div>
			</div>
		</div>
		...
		<div class="mt-16 text-center">
			<div
				class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-emerald-50 text-emerald-800 text-sm font-medium border border-emerald-100"
			>
