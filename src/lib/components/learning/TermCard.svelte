<script lang="ts">
	/** A glossary term: the definition is the card, so there is no cover. */
	import { CARD, TAG, TAG_OFF } from './card';
...
<a href="/learn/glossary/{term.slug}" class="{CARD} flex flex-col bg-white p-5">
	...
	<div class="mt-4 flex items-center justify-between gap-3">
		<span class="{TAG} {TAG_OFF}">{term.topicTitle}</span>
		<span aria-hidden="true" class="text-[13px] text-ecohubs-dark">→</span>
	</div>
</a>
