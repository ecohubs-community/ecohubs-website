<script lang="ts">
	...
	const totalCountries = $derived(new Set(members.map((m) => m.loc.split(' ').pop())).size);
	const totalLangs = $derived(new Set(members.flatMap((m) => m.langs.split(' · '))).size);
...
<div class="relative">
	...
	<div class="relative h-[380px] md:h-[560px] lg:h-[640px]">
		...
		{#each members.slice(0, POSITIONS.length) as member, i}
			...
			{@const ring =
				memberRank[i] === 0 ? 'ring-2 ring-emerald-300/60' : 'ring-1 ring-emerald-400/30'}
			<button
		...
		>
			...
			<span class="hidden sm:flex items-center gap-2">
				<span class="w-5 h-5 rounded-full bg-emerald-500/80 ring-2 ring-emerald-300/50 inline-block"
				></span>Higher contribution
			</span>
...
<div class="mt-8 grid grid-cols-2 md:grid-cols-4 gap-4">
	{#each [{ label: 'Active members', value: totalMembers }, { label: 'Countries', value: totalCountries }, { label: 'Languages', value: totalLangs }, { label: 'XP co-created', value: totalXP > 999 ? `${(totalXP / 1000).toFixed(1)}k` : totalXP }] as stat}
		<div
			class="p-5 rounded-2xl border border-emerald-900/40"
