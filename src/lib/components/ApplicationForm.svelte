<script lang="ts">
	...
	const superform = superForm(data.form, {
		// @ts-expect-error - Zod v3/v4 type compatibility
	...
	// Load page inputs from form when page changes — reading currentPage
	// inside the effect is what registers the dependency for the rerun.
	$effect(() => {
		void currentPage;
		loadPageInputs();
	});
	...
	function loadPageInputs() {
		...
		currentPageQuestions.forEach((q: (typeof visibleQuestions)[number]) => {
			...
			if (q.type === 'scale') {
				inputs[q.id] =
					typeof formValue === 'number' ? formValue : formValue ? Number(formValue) : 0;
	...
	function syncPageToForm() {
		currentPageQuestions.forEach((q: (typeof visibleQuestions)[number]) => {
			...
			if (q.type === 'scale') {
				$form[q.id as keyof typeof $form] = (
	...
	function handleCheckboxChange(questionId: string, option: string, checked: boolean) {
		const current = Array.isArray(pageInputValues[questionId])
			...
			: [];
		const newValue = checked ? [...current, option] : current.filter((v) => v !== option);
...
{#if submitSuccess}
	<div
		class="bg-white rounded-3xl border border-stone-200/70 soft-shadow p-10 md:p-14 text-center"
		in:fade={{ duration: 300 }}
	>
		<div
			class="w-16 h-16 bg-emerald-50 border border-emerald-100 rounded-full flex items-center justify-center mx-auto mb-7"
		>
				...
				{#key currentPage}
					...
					>
						{#each currentPageQuestions as question (question.id)}
							...
							{@const questionValue =
								pageInputValues[question.id] || (question.type === 'scale' ? 0 : '')}
								...
								<div class="mb-4">
									{#if question.type === 'text' || question.type === 'email' || question.type === 'number'}
										<input
											...
											class="w-full px-5 py-3.5 text-base text-ecohubs-deep placeholder:text-stone-400 bg-ecohubs-ivory/60 border rounded-2xl transition-all focus:outline-none focus:bg-white {questionError
												...
												: 'border-stone-300 focus:ring-2 focus:ring-emerald-200 focus:border-emerald-500'}"
										/>
									...
									{:else if question.type === 'select'}
										<select
											...
											aria-invalid={questionError ? 'true' : 'false'}
											class="w-full px-5 py-3.5 text-base text-ecohubs-deep bg-ecohubs-ivory/60 border rounded-2xl transition-all focus:outline-none focus:bg-white {questionError
												? 'border-red-400 focus:ring-2 focus:ring-red-200 focus:border-red-500'
												: 'border-stone-300 focus:ring-2 focus:ring-emerald-200 focus:border-emerald-500'}"
										>
									...
									{:else if question.type === 'radio'}
										...
										>
											{#each question.options || [] as option}
												<label
													class="flex items-center gap-3 px-4 py-3 border rounded-2xl cursor-pointer transition-all {String(
														questionValue
													) === option
														? 'border-emerald-500 bg-emerald-50/50 shadow-sm'
														: questionError
															? 'border-red-200 hover:border-red-300 bg-white'
															: 'border-stone-200 hover:border-emerald-300 hover:bg-ecohubs-ivory/40 bg-white'}"
												>
													<input
														type="radio"
														name={question.id}
									...
									{:else if question.type === 'checkbox'}
										<div class="space-y-2.5" role="group">
											{#each question.options || [] as option}
												...
												<label
													class="flex items-center gap-3 px-4 py-3 border rounded-2xl cursor-pointer transition-all {isChecked
														? 'border-emerald-500 bg-emerald-50/50 shadow-sm'
														: questionError
															? 'border-red-200 hover:border-red-300 bg-white'
															: 'border-stone-200 hover:border-emerald-300 hover:bg-ecohubs-ivory/40 bg-white'}"
									...
									{:else if question.type === 'scale'}
										...
										<div class="scale-input">
											{#if question.scaleLabels?.min || question.scaleLabels?.max}
												<div
													class="flex items-center justify-between mb-3 text-xs tracking-wide text-stone-500 font-story italic"
												>
											...
											{/if}
											<div
												class="grid grid-cols-5 md:grid-cols-10 gap-2"
												role="radiogroup"
												aria-invalid={questionError ? 'true' : 'false'}
											>
												{#each Array.from({ length: 10 }, (_, i) => i + 1) as num}
													...
													<label
														class="relative flex items-center justify-center aspect-square border rounded-xl cursor-pointer transition-all font-serif text-base {isSelected
															? 'border-emerald-500 bg-emerald-600 text-white shadow-sm'
															: questionError
																? 'border-red-200 text-stone-700 hover:border-red-300 bg-white'
																: 'border-stone-200 text-stone-700 hover:border-emerald-300 hover:bg-ecohubs-ivory/40 bg-white'}"
													>
														<input
															type="radio"
															name={question.id}
											...
											<input type="hidden" name={question.id} value={scaleValue} />
										</div>
									{/if}
								</div>
					...
					>
						<div class="flex items-start gap-3">
							...
							<div>
								...
								{#if isRateLimited}
									<p class="text-sm text-red-600 mt-2 font-story italic">
