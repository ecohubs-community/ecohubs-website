<script lang="ts">
	...
	const aboutUs: LinkItem[] = [
		{
			icon: FileText,
			title: 'EcoHubs Manifesto',
			desc: 'Why we are building a regenerative future — read the PDF.',
			href: 'https://ecohubs.community/EcoHubs_Manifesto.pdf'
		},
		{
			icon: Globe,
			title: 'EcoHubs Website',
			desc: 'Our home base — projects, vision and community.',
			href: '/'
		},
		{
			icon: Eye,
			title: 'Our Vision',
			desc: 'Where we are headed and why it matters.',
			href: '/vision'
		},
		{
			icon: Sprout,
			title: 'About RCOS',
			desc: 'The open-source standard for regenerative communities.',
			href: '/rcos'
		},
		{
			icon: UserPlus,
			title: 'Become a Member',
			desc: 'Free, contribution-based — help co-create the RCOS Standard.',
			href: '/membership',
			tag: 'Join us',
			feature: true
		}
	...
	const ecosystem: LinkItem[] = [
		{
			icon: LayoutGrid,
			title: 'RCOS Standard',
			desc: 'The open-source operating system for intentional communities.',
			href: 'https://rcos.ecohubs.community'
		},
		{
			icon: ShieldCheck,
			title: 'Community Resilience Assessment',
			desc: 'Measure and strengthen your community’s resilience.',
			href: '/community-resilience-assessment'
		},
		{
			icon: ShieldCheck,
			title: 'Stress-Test Self-Assessment',
			desc: 'Reveal your community’s hidden weaknesses & resilience gaps before they become real-world failures',
			href: 'https://rcos.ecohubs.community/articles/rcos-stress-tests/self-assessment'
		},
		{
			icon: Compass,
			title: 'Community Suitability Index',
			desc: 'Find out where a regenerative community could thrive.',
		...
		},
		{
			icon: Vote,
			title: 'VoteCast',
			desc: 'Collective decision-making, made transparent.',
	...
	const podcastChannels = [
		{
			...
			href: 'https://podcasts.apple.com/podcast/id1884349817',
			domain: 'podcasts.apple.com'
		},
		{
			label: 'Spotify',
			href: 'https://open.spotify.com/show/4pAswyGtil1mLSrhN38bps',
			domain: 'open.spotify.com'
		},
		{
			label: 'YouTube',
			href: 'https://www.youtube.com/watch?v=NuF4LobS0lA&list=PLFyJdVEi7LlNZdh0o_2eoyZ3XlUVLkjBe',
			domain: 'youtube.com'
		},
		{
			label: 'Amazon Music',
			href: 'https://music.amazon.com/podcasts/210f533e-0b5d-495b-933d-3663bc6e5cb7',
			domain: 'music.amazon.com'
		},
		{
			label: 'Podcast Index',
			href: 'https://podcastindex.org/podcast/7756027',
			domain: 'podcastindex.org'
		},
		{
			label: 'Castamatic',
			href: 'https://castamatic.com/itunes/1884349817',
			domain: 'castamatic.com',
			icon: 'https://castamatic.com/img/favicon-256.png'
		},
		...
		{
			label: 'CurioCaster',
			href: 'https://curiocaster.com/podcast/pi7756027',
			domain: 'curiocaster.com'
		},
		{ label: 'Pocket Casts', href: 'https://pca.st/itunes/1884349817', domain: 'pocketcasts.com' },
		{
			label: 'Podcast Addict',
			href: 'https://podcastaddict.com/feed/https://media.rss.com/the-regenerative-future-podcast/feed.xml',
			domain: 'podcastaddict.com'
		},
		{
			label: 'Podcast Guru',
			href: 'https://app.podcastguru.io/podcast/1884349817',
			domain: 'podcastguru.io'
		},
		{
			label: 'PodLP',
			href: 'https://link.podlp.app/938e3a51-03ab-55f2-9491-2daaeee027ac',
			domain: 'podlp.app'
		},
		{
			label: 'Podverse',
			href: 'https://api.podverse.fm/api/v1/podcast/podcastindex/7756027',
			domain: 'podverse.fm'
		}
	];
	...
	const shareTargets = [
		...
		},
		{
			label: 'Email',
			icon: 'tabler:mail',
			href: `mailto:?subject=${enc('EcoHubs Community')}&body=${enc(`${shareText}\n${shareUrl}`)}`
		}
	];
...
{#snippet sectionLabel(text: string)}
	<div class="mt-10 mb-4 flex items-center gap-3 px-0.5">
		<span class="text-[11px] font-semibold uppercase tracking-[0.22em] text-ecohubs-light"
			>{text}</span
		>
	...
	>
		...
		<span class="flex min-w-0 flex-1 flex-col">
			<span class="flex items-center gap-2">
				<span
					class="text-[15px] font-semibold leading-tight {item.feature
						? 'text-ecohubs-base'
						: 'text-ecohubs-deep'}"
				>
					{item.title}
				...
				{#if item.tag}
					<span
						class="text-[10px] font-bold uppercase tracking-[0.12em] px-2 rounded-full text-white {item.feature
							? 'bg-white/10'
							: 'bg-ecohubs-accent'}"
					>
						{item.tag}
			...
			<span
				class="mt-1 text-[12.5px] leading-snug {item.feature
...
>
	...
	<div class="relative z-[2] mx-auto max-w-[600px] px-5 pb-20 pt-14">
		...
		<div
			class="overflow-hidden rounded-2xl bg-ecohubs-base p-1.5 shadow-[0_18px_40px_-24px_rgba(0,0,0,0.6)]"
		>
		...
		<div
			class="rounded-2xl bg-gradient-to-br from-ecohubs-dark to-ecohubs-deep p-5 shadow-[0_14px_30px_-22px_rgba(0,0,0,0.5)] ring-1 ring-ecohubs-light/15"
		>
			<!-- Badges -->
			<div class="flex flex-wrap gap-2">
				<span
					class="inline-flex items-center gap-1.5 rounded-full bg-white/10 px-2.5 py-1 text-[11px] font-semibold text-ecohubs-light"
				>
		...
		<div class="overflow-hidden rounded-2xl shadow-[0_18px_40px_-24px_rgba(0,0,0,0.6)]">
			<iframe
				...
				scrolling="no"
			>
				<a href="https://rss.com/podcasts/the-regenerative-future-podcast"
					>The Regenerative Future Podcast | RSS.com</a
				>
			</iframe>
		</div>
		<p class="mt-5 mb-3 px-0.5 text-[11px] uppercase tracking-[0.22em] text-ecohubs-light">
			Also available on
		</p>
			...
			{#each visibleChannels as ch (ch.label)}
				...
				>
					<img
						...
						width="28"
						height="28"
						class="h-7 w-7 flex-none rounded-md"
						loading="lazy"
					/>
					<span class="flex min-w-0 flex-col leading-tight">
						<span class="text-[9.5px] uppercase tracking-[0.14em] text-ecohubs-base/45"
							>Listen on</span
						>
		...
		{#if podcastChannels.length > PODCAST_PREVIEW}
			<div class="mt-3 flex justify-center">
				...
				>
					{showAllChannels ? 'Show less' : `Show ${podcastChannels.length - PODCAST_PREVIEW} more`}
					<ChevronDown
						size={15}
		...
		<div class="flex flex-col gap-3">
			...
			<details
				class="group overflow-hidden rounded-2xl bg-ecohubs-base shadow-[0_14px_30px_-22px_rgba(0,0,0,0.5)]"
			>
				<summary
					class="flex cursor-pointer list-none items-center gap-4 p-4 [&::-webkit-details-marker]:hidden"
				>
					<span
						class="grid h-11 w-11 flex-none place-items-center rounded-xl bg-ecohubs-ivory text-ecohubs-dark"
					>
						<MessageSquare size={22} strokeWidth={1.7} />
					</span>
					<span class="flex min-w-0 flex-1 flex-col">
						<span class="text-[15px] font-semibold leading-tight text-ecohubs-deep"
		...
		{#if featuredPost}
			...
			>
				...
				<span class="block p-4">
					...
					<span
						class="mt-1.5 mb-1.5 block text-[17px] font-semibold leading-tight text-ecohubs-deep"
					>
						{featuredPost.title}
					</span>
					<span class="line-clamp-3 text-[13px] leading-normal text-ecohubs-muted"
						>{featuredPost.excerpt}</span
					>
				</span>
			</a>
					...
					{#each morePosts as post (post.slug)}
						...
						>
							...
							<span class="min-w-0 flex-1 py-3.5 pr-4">
								<span
									class="text-[10.5px] font-semibold uppercase tracking-[0.1em] text-ecohubs-primary"
								>
									{formatDate(post.date)}
								</span>
								<span
									class="mt-1.5 mb-1.5 block text-[15px] font-semibold leading-tight text-ecohubs-deep"
								>
									{post.title}
								</span>
								<span class="line-clamp-2 text-[12.5px] leading-normal text-ecohubs-muted"
									>{post.excerpt}</span
								>
							</span>
						</a>
...
{#if qrOpen}
	<div class="fixed inset-0 z-50 flex items-center justify-center p-6">
		...
		<div class="relative z-10 flex flex-col items-center">
			...
			/>
			<p class="mt-4 text-center text-[13px] text-white/80">
				Scan to open this page on another phone
			</p>
			<button
				type="button"
				onclick={() => (qrOpen = false)}
