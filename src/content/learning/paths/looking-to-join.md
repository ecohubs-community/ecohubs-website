---
type: path
title: I want to join a community
slug: looking-to-join
summary: For people who want to live in one rather than start one.
audience: For people who want to join one, not start one
image: learning-paths/join-community.webp
imageAlt: >-
  Four people embracing in a shared vegetable garden, with timber and
  green-roofed homes behind them and a rucksack set down on the path.
outcomes:
  - Say what an intentional community actually is, and what it is not
  - Tell the main forms apart well enough to know which suits you
  - Ask a community the questions that reveal how it really decides
  - Judge a visit honestly rather than hopefully
pairs:
  - label: Cohousing vs ecovillage
    href: /learn/compare/cohousing-vs-ecovillage
  - label: Glossary — consent
    href: /learn/glossary/consent
status: published
updated: 2026-08-05
steps:
  - guide: intentional-communities
    lesson: what-is-an-intentional-community
  - guide: intentional-communities
    lesson: forms-village-cohousing-commune
endsAt:
  label: Find a community that fits
  href: /seeking
---


