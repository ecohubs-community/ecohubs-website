...
- **Icons are Iconify (`@iconify/svelte`, tabler set)** — no hand-written `<svg>` in learning code. Note it renders nothing server-side and fetches icon data from `api.iconify.design` after hydration, so anything sized by an icon needs its box reserved (see `RailIcon.svelte`).
