...
- **The hub is descriptive; RCOS is normative. What belongs in an [`<Rcos>`](./src/lib/components/learning/Rcos.svelte) card is the _attribution_, not the subject.** The rule is about voice, not topic: **"RCOS requires X", "the specification says MUST", a clause citation** — those go in a card, because a survey that quotes a standard as its own conclusion has become advocacy. The underlying practical point, written in the hub's own reporting voice (_what communities that handle this well do differently_, _the question worth asking on a visit_), belongs in the **body**, and needing RCOS to have thought of it first does not change that. Layer URLs come from [`learning/rcos.ts`](./src/lib/learning/rcos.ts), pinned to v0.1. **A section link needs the real heading `id`** — check it (`curl -sL <layer-url> | grep -o 'id="[0-9][^"]*"'`), never guess the slug; a summarising fetch got the format wrong once already. `pnpm links` checks `href="…"` too, so a card's citation is covered.
- **Inside a card, RFC 2119 keywords are the content — copy the level, not the gist.** RCOS distinguishes `MUST` from `MAY` deliberately, and a card that flattens the two reports a standard that does not exist. Three shapes to watch, each of which was got wrong once: a mandatory _outcome_ with an optional _menu_ ("elevated safeguards MUST be applied, which MAY include…" — the measures are choices); **"at minimum"**, which sets a floor, not a ceiling (four membership states must exist; a community may add more); and **"(if any)"** on a list item, which makes that item conditional while its neighbours are required. Cite the subclause (`§6.3.3`) rather than only the section, so the claim is checkable at the source.
- **`<Rcos>` is part of the deep layer, so it may not carry anything the article needs.** The test before writing one: if the point matters at standard depth, it belongs in the body, per the bullet above — at which point it is simply a better article. What is left for the card is the specification's own framing: which clause, how strictly it is worded, where to read it. A card that would leave a hole in the article if a reader never opened deep mode is written wrong.
- **Content is British English** — _organisation_, _recognise_, _colour_, _behaviour_, _programme_, _co-operative_, _afterwards_. Consistent across ~76 files; do not "correct" it toward American spelling.
...
- **`pnpm links` checks every external citation** in the hub (~50 URLs — `url:`, markdown links, and `href="…"` on components). Not in CI — it needs the network, so as a build gate it would fail because somebody else's server had a bad afternoon. Run it before a content release. It reports three outcomes, not two: **401, 403 and 429 mean a server refusing a script**, not a dead page (Twin Oaks, Fannie Mae and Wiley all block bots and are fine in a browser), so those are counted separately and do not fail the run. **Every other 4xx/5xx, and every request that fails outright, exits non-zero.** Redirects are reported rather than failed so you can check the citation still names the right publisher — that is how the Cohousing Association's move to `cohousingalliance.org` was caught.
- **A citation URL must be a public address, and `check-links.ts` enforces it at connection time.** Content arrives in pull requests, so without this a markdown file could point `pnpm links` at `169.254.169.254` or a loopback port on the machine of whoever runs it. Two halves, and **both are needed**: `validatingLookup()` is passed to `http.request` as its `lookup`, so it is the resolution the socket uses and a rebinding name server gets one answer rather than two; `assertRequestable()` covers IP literals, which never reach a lookup at all. That is also why the script uses `node:http` rather than `fetch` — `fetch` gives no way to own the lookup without adding undici. Redirects are followed by hand so every hop goes through both. Specs in `scripts/check-links.spec.ts`; **verify a change by planting a URL in a content file and running it**, which is how the literal-address gap was found.
