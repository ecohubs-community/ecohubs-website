...
- **The hub is descriptive; RCOS is normative — keep the two voices apart.** Articles report what communities actually do and mark what could not be verified. The RCOS Standard says what a community MUST write down. A normative claim goes in an [`<Rcos>`](./src/lib/components/learning/Rcos.svelte) card, never in running prose, or a survey quietly becomes advocacy. Layer URLs come from [`learning/rcos.ts`](./src/lib/learning/rcos.ts) and are pinned to v0.1. **A section link needs the real heading `id`** — check it (`curl -sL <layer-url> | grep -o 'id="[0-9][^"]*"'`), never guess the slug; a summarising fetch got the format wrong once already. `pnpm links` checks `href="…"` too, so a card's citation is covered.
- **`<Rcos>` is part of the deep layer, so it may not carry anything the article needs.** The test before writing one: if the point matters at standard depth, it belongs in the **body**, in the hub's own voice, whether or not RCOS is named — at which point it is simply a better article. What is left for the card is the specification's own framing: which clause, how strictly it is worded, where to read it. A card that would leave a hole in the article if a reader never opened deep mode is written wrong.
