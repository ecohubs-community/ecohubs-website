...
- **A citation URL must be a public address, and `check-links.ts` enforces it** before the request and on every redirect hop (`assertPublic()`, with a spec). Content arrives in pull requests, and without the guard a markdown file could point `pnpm links` at `169.254.169.254` or a loopback port on the machine of whoever runs it. It stops short of DNS-rebinding defence, which would need a custom dispatcher — the tradeoff is written up at the function.
